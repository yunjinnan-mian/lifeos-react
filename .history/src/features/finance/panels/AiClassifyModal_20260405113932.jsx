...
const [apiBase, setApiBase] = useState(saved.apiBase || 'http://localhost:11434');
const [apiKey,  setApiKey]  = useState(saved.apiKey  || '');
const [model,   setModel]   = useState(saved.model   || 'phi3');
...
if (!isLocal && !apiKey.trim()) { setErrorMsg('请填写 API Key（本地 Ollama 不需要填）'); return; }        if (pending.length === 0) { setErrorMsg('没有待分类的条目（已有分类的行自动跳过）'); return; }
...
const endpoint = `${apiBase.trim().replace(/\/$/, '')}/api/generate`;
