...
export function useFifoData() {
  ...
  const weeklyData = useMemo(() => {
    ...
    const currentYearTxs = allTxs.filter(tx => {
      ...
      return parseInt(tx.date.substring(0, 4), 10) === activeYear && tx.cat1 !== '其他' && !(tx.cat2 || '').includes('平账');
    });
    ...
    currentYearTxs.forEach(tx => {
      ...
      txsByWeek[wKey].sAllCents = (txsByWeek[wKey].sAllCents || 0) + amountCents; // 新增：无差别记录这周所有的支出总和
    ...
    while (true) {
      ...
      // ISO 周定义：该周的星期四如果已经到了下一年，说明今年的周已经排完了
      const thursday = new Date(wStart.getTime() + 3 * 864e5);
      if (thursday.getFullYear() > activeYear) break;
      ...
      weeks.push({
        ...
        sAll: (txsByWeek[wKey]?.sAllCents || 0) / 100, 
