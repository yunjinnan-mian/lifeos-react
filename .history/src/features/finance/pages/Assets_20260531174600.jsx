...
function fmtDate(d) {
    ...
    let res = `${parseInt(m)}月${parseInt(day)}日`;
    if (timePart) {
...
const Assets = memo(function Assets() {
    const { showToast, data } = useFinance();
    ...
    const rows = useMemo(() => {
        ...
        const normalizeDateTimeStr = (dStr) => {
            if (!dStr) return '';
            let cleanStr = dStr.replace('T', ' ').trim();
            let [datePart, timePart] = cleanStr.split(' ');

            const dParts = datePart.split('-');
            const yyyy = dParts[0];
            const mm = dParts[1]?.padStart(2, '0') || '00';
            const dd = dParts[2]?.padStart(2, '0') || '00';

            // 如果是旧快照或只有日期的记录，默认设为当天的最后一秒，以囊括全天账单
            if (!timePart) timePart = '23:59:59';
            const tParts = timePart.split(':');
            const hh = tParts[0]?.padStart(2, '0') || '00';
            const min = tParts[1]?.padStart(2, '0') || '00';
            const ss = tParts[2]?.padStart(2, '0') || '00';
        ...
        return sorted.map((snap, idx) => {
            ...
            if (idx > 0) {
                ...
                if (txs && txs.length > 0) {
                    const normPrevDate = normalizeDateTimeStr(prev.date);
                    const normSnapDate = normalizeDateTimeStr(snap.date);

                    // 1. 严格使用补零后的纯日期进行区间对比
                    const periodTxs = txs.filter(r => {
                        if (!r.date) return false;
                        const normRecordDate = normalizeDateTimeStr(r.date);
                    ...
                    <div className="rounded-2xl border-2 border-primary bg-surface-bright shadow-[0_0_20px_rgba(24,31,33,0.05)] p-6 mb-8 animate-fade-in-down">
                        <div className="flex justify-between items-center mb-6 pb-4 border-b border-surface-variant border-dashed">
                            <div className="flex items-center gap-4">
                                <h3 className="text-lg font-bold text-primary">{editingSnapshotId ? '📸 修改快照' : '📸 记录快照'}</h3>
                                <input 
type="datetime-local" value={formDate} onChange={e => setFormDate(e.target.value)}
                                    className="bg-transparent text-sm font-mono-num font-bold text-on-surface-variant outline-none cursor-pointer"
                                />
                            </div>
                            <button onClick={() => { setIsRecording(false); setEditingSnapshotId(null); }} className="text-outline hover:text-error transition-colors"><i className="ri-close-line text-xl"></i></button>
                        </div>
                                        ...
                                        <input 
                                            ...
                                            onKeyDown={e => {
                                                if (e.key === 'Enter') {
                                                    ...
                                                    handleSaveSnapshot({ id: editingSnapshotId || `snap_${Date.now()}`, date: formDate, balances: final });
                                                }
                                            }}
                        ...
                        >
                            ...
                            >
                                ...
                                <div className="flex items-center gap-8 mt-2 sm:mt-0">
                                    {row.actualDiff != null && (
                                        <div className="flex items-center gap-4 sm:gap-6 border-l border-surface-variant pl-4 sm:pl-6 ml-2">
                                            {/* 1. 账单明细总和 */}
                                            {row.recordedDiff != null && (
                                                <div className="flex-col items-end hidden sm:flex">
                                                    <span className="text-[10px] text-outline uppercase mb-0.5">账单统计</span>
                                                    <span className={`font-mono-num font-bold text-sm ${row.recordedDiff >= 0 ? 'text-secondary' : 'text-on-surface-variant'}`}>
                                                        {fmtDiff(row.recordedDiff)}
                                                    </span>
                                                </div>
                                            )}
                                            ...
                                            <div className="flex flex-col items-end">
                                                <span className="text-[10px] text-outline uppercase mb-0.5">实际变动</span>
                                                <span className={`font-mono-num font-bold text-base ${row.actualDiff >= 0 ? 'text-secondary' : 'text-error'}`}>
                                                    {fmtDiff(row.actualDiff)}
                                                </span>
                                            </div>

                                            {/* 3. 平账误差 */}
                                            {row.untrackedDiff != null && (
                                                <div className={`flex flex-col items-end px-3 py-1 rounded-lg ${Math.abs(row.untrackedDiff) < 0.05 ? 'bg-secondary/10' : 'bg-error/10'}`}>
                                                    <span className={`text-[10px] uppercase mb-0.5 ${Math.abs(row.untrackedDiff) < 0.05 ? 'text-secondary' : 'text-error'}`}>
                                                        {Math.abs(row.untrackedDiff) < 0.05 ? '完美平账' : '平账误差'}
                                                    </span>
                                                    <span className={`font-mono-num font-bold text-sm ${Math.abs(row.untrackedDiff) < 0.05 ? 'text-secondary' : 'text-error'}`}>
                                                        {fmtDiff(row.untrackedDiff)}
                                                    </span>
                                                </div>
                                            )}
                                        </div>
