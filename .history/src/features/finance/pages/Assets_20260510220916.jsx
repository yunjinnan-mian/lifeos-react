    ...
    return (
        <div style={{ paddingBottom: 40, width: '100%' }}>
            ...
            <style>{`
                ...
                .glass-divider { width: 1px; height: 16px; background: #cbd5e1; margin: 0 2px; }
            ...
            {chartRender && (
                <div style={{ position: 'relative', height: 80, marginBottom: 12, borderRadius: 8, overflow: 'hidden' }}>
                    ...
                    {hoverChartIdx != null && (() => {
                        ...
                        if (hoverChartIdx <= 1) transform = 'translateX(0)';
                        else if (hoverChartIdx >= lastIdx - 1) transform = 'translateX(-100%)';
                        return (
                            <div style={{
            ...
            {/* 表格区 */}
            <div style={{ position: 'relative', borderRadius: 4, overflow: 'hidden', border: `1px solid ${BORDER_COLOR}` }}>
                ...
                >
                    <table style={{ width: '100%', borderCollapse: 'collapse', background: '#fff', fontSize: 13, tableLayout: 'auto' }}>
                        <thead>
                            <tr>
                                {/* ① 合计列：右对齐 */}
                                <th style={{ ...TH_STYLE, ...TH_NUM, color:'#10b981', minWidth: 90 }}>合计</th>
                                {/* ② 日期列：居中 */}
                                <th style={{ ...TH_STYLE, width: isEditMode ? 140 : 110, textAlign: 'center' }}>日期</th>

                                {accounts.map((acc, idx) => (
                                    <th key={acc.id} style={{ ...TH_STYLE, minWidth: 110, textAlign: 'center' }}>
                                ...
                                {/* ③ 变动列：右对齐 */}
                                <th style={{ ...TH_STYLE, ...TH_NUM, minWidth: 100 }}>变动</th>
                            </tr>
                        </thead>
                        ...
                        <tbody>
                            {displayRows.map((row, idx) => {
                                ...
                                const rowBorder = isLatestRow ? '2px solid #bbf7d0' : undefined;
                                ...
                                return (
                                    <tr
                                        key={row.id}
                                        style={{ background: rowBg, outline: rowBorder ? rowBorder : undefined, transition:'background 0.1s' }}
                                    ...
                                    >
                                        {/* 合计：右对齐 + 等宽 */}
                                        <td style={{ ...TD_STYLE, ...TD_NUM, fontWeight:'bold', background: isLatestRow ? 'transparent' : '#fafafa', color: isLatestRow ? '#166534' : '#0f172a' }}>
                                            {fmt(row.total)}
                                        </td>

                                        {/* 日期：居中 */}
                                        <td style={{ ...TD_STYLE, textAlign:'center', whiteSpace:'nowrap', color: textColor, fontWeight: fw }}>
                                            {isEditMode ? (
                                        ...
                                        {accounts.map(acc => {
                                            ...
                                            return (
                                                <td
                                                    ...
                                                    style={{ ...TD_STYLE, ...TD_NUM, cursor: isEditMode ? 'not-allowed' : 'text', position: 'relative' }}
                                                >
                                                    ...
                                                    {isEditing && !isEditMode && (
                                                        <input
                                                            ...
                                                            style={{
                                                                ...
                                                                margin: 0, textAlign: 'right', paddingRight: 12,
