...
// ── 派生时间状态的工具函数（免疫一切日期干扰版） ───────────────────

function cleanDate(d) {
    if (!d) return '';
    // 将可能出现的斜杠替换为横杠，并严格截取前 10 位 (YYYY-MM-DD)
...
function getNewestMonth(txs) {
    ...
    const newest = txs.reduce((max, t) => {
        const cd = cleanDate(t.date);
...
function getMonthsOfYear(txs, year) {
    ...
    txs.forEach(t => {
        const cd = cleanDate(t.date);
        if (cd.startsWith(year)) set.add(cd.slice(0, 7));
    });
    return [...set].sort();
}
...
const Dashboard = memo(function Dashboard({ onJumpToCategory }) {
    // 1. 将原本的 data 重命名为 rawData 拦截下来
    const { data: rawData } = useFinance();

    // 2. 核心免疫层：清洗掉所有附带的时间和斜杠，给下层提供纯净的 YYYY-MM-DD
    // 这样底层的 Heatmap、MatrixTable 统统不需要改代码就能复活
    const data = useMemo(() => ({
        ...rawData,
        txs: rawData.txs.map(t => ({
            ...t,
