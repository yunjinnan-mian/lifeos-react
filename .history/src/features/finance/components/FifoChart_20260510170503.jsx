  ...
  useEffect(() => {
    ...
    ecInstance.current.setOption({
      ...
      series: [
        ...
        { name: '预算内消耗', type: 'bar', stack: 's', barWidth: '40%', z: 5, itemStyle: { color: C.base, borderRadius: [2, 2, 0, 0] }, data: base },
        ...
        { name: '本周预算', type: 'line', step: 'middle', symbol: 'none', z: 6, lineStyle: { color: C.budget, width: 1, type: 'dashed', opacity: 0.5 }, data: weeklyData.map(x => x.b) },
        { name: '总支', type: 'line', smooth: 0.3, symbol: 'none', z: 8, lineStyle: { color: C.gold, width: 2, type: 'solid' }, data: weeklyData.map(x => x.sAll) },
        { name: '生活开销', type: 'line', smooth: 0.3, symbol: 'none', z: 10, lineStyle: { color: '#d1a054', width: 2, type: 'solid' }, areaStyle: { color: new echarts.graphic.LinearGradient(0, 0, 0, 1, [{ offset: 0, color: 'rgba(209, 160, 84, 0.2)' }, { offset: 1, color: 'rgba(209, 160, 84, 0)' }]) }, data: weeklyData.map(x => x.s) },
      ],
    });

    // ✨ 联动高亮魔法：鼠标悬浮超支周时，点亮被掏空的往期柱子
    ecInstance.current.off('mouseover');
    ecInstance.current.off('mouseout');

    ecInstance.current.on('mouseover', (params) => {
      const idx = params.dataIndex;
      if (consumedMap[idx] && consumedMap[idx].sources.length > 0) {
        ecInstance.current.dispatchAction({
          type: 'highlight',
          seriesIndex: 2, // '动用结余' 所在的 series index
          dataIndex: consumedMap[idx].sources
        });
      }
    });

    ecInstance.current.on('mouseout', () => {
      ecInstance.current.dispatchAction({ type: 'downplay' });
    });
  }, [weeklyData]);
