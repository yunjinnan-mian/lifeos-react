...
export default function ExplorationModal({ isOpen, zone, onClose }) {
  ...
  useEffect(() => {
    ...
    if (entries.some(e => e.id === newEntryId)) {
      notebookRef.current?.scrollTo({ top: 0, behavior: 'smooth' });
    }
  }, [newEntryId, entries]);
...
// 条目卡片
// ============================================================
function EntryCard({ entry, showDate, isNew, dateRef }) {
  ...
  return (
    <div
      className={`exp-entry-card${isNew ? ' is-new' : ''}`}
      ref={dateRef}
