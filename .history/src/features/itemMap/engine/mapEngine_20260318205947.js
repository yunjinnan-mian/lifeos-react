 ...
 */
import { CONFIG, T, BASE_COLOR, EXPLORE_BASE_COLOR, ZONE_CATEGORY_BITS, ZONE_TYPES } from '../../../lib/config.js';
...
function getIslandTiles(zone, itemsByZone) {
  ...
  // High nibble = zone category (0x00 = items, 0x10 = exploration)
  // Low  nibble = tile type     (T.DEEP … T.FOREST, values 0–5)
  // Max packed value: 0x15 = 21, safe for Uint8Array (0–255)
  // Old zones without `type` field default to 0x00 (items) — no data migration needed
  const catBit = ZONE_CATEGORY_BITS[zone.type] ?? 0x00;
  const result = [];
  ...
  for (let dy = -maxR; dy <= maxR; dy++) {
    for (let dx = -maxR; dx <= maxR; dx++) {
      ...
      if (d <= edgeR) {
        const pct = d / edgeR;
        let tiletype;
        if (pct < 0.30) tiletype = rich > 0.6 ? T.FOREST : rich > 0.2 ? T.DGRASS : T.GRASS;
        else if (pct < 0.60) tiletype = rich > 0.35 ? T.DGRASS : T.GRASS;
        else if (pct < 0.80) tiletype = T.GRASS;
        else tiletype = T.SAND;
        result.push({ gx: cx + dx, gy: cy + dy, type: catBit | tiletype });
      } else if (d <= edgeR + SW) {
        result.push({ gx: cx + dx, gy: cy + dy, type: catBit | T.SHALLOW });
      }
    }
  }
  return result;
...
function drawTile(gx, gy, packedType, sx, sy, tw, th) {
  // Unpack packed byte: high nibble = zone category, low nibble = tile type
  const isExplore = (packedType & 0x10) !== 0;
  const type = packedType & 0x0F;
  const bc = isExplore ? EXPLORE_BASE_COLOR : BASE_COLOR;
  ...
  switch (type) {
    ...
    case T.SAND: {
      if (isExplore) {
        // 羊皮纸沙滩：暖棕杂点，无金色光斑
        if (h % 3 === 0) r((h >> 1) % (TS - 1), (h >> 4) % (TS - 1), 1, 1, '#e8d0a8');
        if (h % 4 === 0) r((h >> 3) % (TS - 1), (h >> 6) % (TS - 1), 1, 1, '#c0a070');
        if (h % 6 === 0) r((h >> 2) % (TS - 2), (h >> 5) % (TS - 2), 2, 1, '#d4b080');
        if (h % 26 === 0) { r((h >> 2) % 4 + 1, (h >> 5) % 4 + 1, 3, 2, '#f0e4c8'); r((h >> 2) % 4 + 2, (h >> 5) % 4 + 2, 1, 1, '#d8c898'); }
      } else {
    ...
    case T.GRASS: {
      if (isExplore) {
        // 鼠尾草草地：小白花点缀，观察日记风格
        if (h % 7 === 0) r(0, 0, TS, TS, '#68a058');
        r(0, TS - 2, TS, 2, '#3a7840');
        if (h % 2 === 0) { r((h >> 1) % (TS - 1), TS - 4, 1, 4, '#428850'); r((h >> 3) % (TS - 1), TS - 3, 1, 3, '#4e9c28'); }
        if (h % 3 === 0) r((h >> 4) % (TS - 1), TS - 5, 1, 2, '#80c750');
        // 小白花（替代物品岛的黄花/粉花）
        if (h % 24 === 0) { r((h >> 2) % 4 + 1, (h >> 6) % 3 + 2, 2, 2, '#f0f0e0'); r((h >> 2) % 4 + 2, (h >> 6) % 3 + 1, 1, 1, '#fff'); }
        if (h % 31 === 0) r((h >> 3) % 4 + 1, (h >> 7) % 3 + 2, 2, 2, '#c8d890');
      } else {
    ...
    case T.DGRASS: {
      if (isExplore) {
        // 深鼠尾草 + 苔藓斑：像青苔覆石，植物标本感
        if (h % 7 === 0) r(0, 0, TS, TS, '#3a7848');
        r(0, TS - 2, TS, 2, '#1e5038');
        if (h % 2 === 0) { r((h >> 1) % (TS - 1), TS - 5, 1, 5, '#1e6038'); r((h >> 3) % (TS - 1), TS - 6, 1, 6, '#287838'); }
        if (h % 3 === 0) r((h >> 4) % (TS - 1), TS - 7, 1, 3, '#50b038');
        if (h % 3 === 1) r((h >> 5) % (TS - 1), TS - 4, 1, 1, '#80c750');
        if (h % 38 === 0) { r((h >> 2) % 3 + 1, (h >> 7) % 3 + 1, 3, 3, '#2a6040'); r((h >> 2) % 3 + 1, (h >> 7) % 3 + 1, 1, 1, '#3a8050'); }
      } else {
    ...
    case T.FOREST: {
      if (isExplore) {
        // 茶绿密林：树干保留，冠色换成暗茶绿
        r(0, 0, TS, TS, h % 3 === 0 ? '#143828' : '#1a4a38');
        if (h % 4 < 2) {
          const tx = h % 3 === 0 ? 1 : 0;
          r(tx + TS / 2 - 1 | 0, TS - 3, 2, 3, '#6B3710'); r(tx + TS / 2 | 0, TS - 3, 1, 3, '#8a5020');
          r(tx + 1, TS - 6, TS - 2, 4, '#0a2e1e'); r(tx + 2, TS - 6, TS - 4, 3, '#1a6040'); r(tx + 3, TS - 6, TS - 6, 1, '#2a8050');
          r(tx + 2, TS - 9, TS - 4, 4, '#082818'); r(tx + 3, TS - 9, TS - 6, 3, '#145040'); r(tx + 4, TS - 9, TS - 8, 1, '#1e6848');
          r(tx + TS / 2 - 2 | 0, 1, 4, TS - 9, '#0a2e1e'); r(tx + TS / 2 - 1 | 0, 2, 2, TS - 10, '#145040'); r(tx + TS / 2 | 0, 2, 1, 2, '#2a7048');
        } else {
          if (h % 5 === 0) r((h >> 2) % (TS - 2) + 1, (h >> 5) % (TS - 4) + 1, 2, 1, '#1a5038');
          if (h % 7 === 0) r((h >> 3) % (TS - 2) + 1, (h >> 6) % (TS - 4) + 1, 3, 2, '#2a6840');
        }
      } else {
...
function drawZoneLabels() {
  ...
  _zones.forEach(zone => {
    const isExplore = zone.type === ZONE_TYPES.EXPLORATION;
    ...
    // Emoji：探索岛无自定义 emoji 时默认 🔭
    const emoji = zone.emoji || (isExplore ? '🔭' : '📦');
    _ctx.font = '16px serif';
    _ctx.fillStyle = 'rgba(0,0,0,0.55)';
    _ctx.fillText(emoji, spx + 1, spy + 1);
    _ctx.fillStyle = '#ffffff';
    _ctx.fillText(emoji, spx, spy);

    // 名称：探索岛用暖琥珀色，物品岛用浅绿色
    _ctx.font = "bold 10px 'Zpix',monospace";
    _ctx.fillStyle = 'rgba(0,0,0,0.7)';
    _ctx.fillText(zone.name, spx + 1, spy + 15);
    _ctx.fillStyle = isExplore ? '#f5dfa0' : '#e8f8e0';
    _ctx.fillText(zone.name, spx, spy + 14);

    // 副标题：物品岛显示物品数，探索岛显示「探索」标识
    _ctx.font = "9px 'Zpix',monospace";
    if (isExplore) {
      _ctx.fillStyle = 'rgba(245,210,120,0.65)';
      _ctx.fillText('探索', spx, spy + 25);
    } else if (zi.length > 0) {
